...
function ThemeButton({ theme, onSelect }: { theme: string | null; onSelect: (theme: string) => void }) { if (!theme) return <AnalysisBadge value={null}/>; const select = () => onSelect(theme); return <button type="button" onClick={select} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); select(); } }} aria-label={`View supporting evidence for ${theme}`} className="inline-block cursor-pointer rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-bold text-amber-700 transition hover:bg-amber-100 hover:text-amber-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">{theme}</button>; }
